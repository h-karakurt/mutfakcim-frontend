...
export default function ForgetPassword() {
  ...
  function HandlePost(event) {
    ...
    if (passWord !== passAgain?.current?.value) {
      Swal.fire({
        title: "Hata",
        text: `Tekrar girilen şifre hatalı!`,
        icon: "error",
        confirmButtonText: "Tekrar Deneyelim",
      }).then(() => {setLoading(false)})
    } else {
      const passData = JSON.stringify({ password: passWord });
      ...
      axios({
        ...
        data: passData,
          ...
          Swal.fire({
            title: "Tamamdır!",
            text: `Şifreni başarıyla değiştirdik!`,
            icon: "success",
            confirmButtonText: "Oley",
          }).then(() => {
            navigate("/auth/login");
          });
          setLoading(false);
      ...
      <Row>
        <Col xl={{order: "first" , span: "6"}} lg={{order: "first" , span: "6"}} md={{order: "last" , span: "12"}} sm={{order: "last" , span: "12"}} xs={{order: "last" , span: "12"}} className="left-panel">
          <Form>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control type="password" ref={passAgain} />
            </Form.Group>
        ...
        <Col xl={{order: "last" , span: "6"}} lg={{order: "last" , span: "6"}} md={{order: "first" , span: "12"}} sm={{order: "first" , span: "12"}} xs={{order: "first" , span: "12"}} className="right-panel">
          <div className="mt-5">
            ...
            <Text className="mt-5 mb-5">Yeni şifrenizi belirleyin</Text>
          </div>
        </Col>
      </Row>
    </CenteredDiv>
